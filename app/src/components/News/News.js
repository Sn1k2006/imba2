...
const News = ({data, meta, get, count_today, onEndReached, navigation}) => {
  ...
  const renderHeader = () => {
    return <Text style={[Styles.page_padding, Styles.title, {paddingHorizontal: 16, paddingBottom: 20}]}>{translate('News')} <Text
  ...
  return (
      <FlatList
        ...
        data={data}
        extraData={data}
        renderItem={({item}) => <NewsListItem data={item} onPress={handleRoute}/>}
